  ...
  _addEvent(payload: EventPayload): void {
    ...
    switch (payload.name) {
      ...
      case MetadataEvent.ThreadName: {
        if (payload.args.name) {
          process.threadById(payload.tid)._setName(payload.args['name']);
        } else if (payload.ph === 'M' && process.id() === payload.tid && process.id() !== 0) {
          process.threadById(payload.tid)._setName('Main');
        }
        break;
      }
    }
...
export const DevToolsMetadataEventCategory = 'lynx';
...
export class Process extends NamedObject {
  ...
  sortedThreads(): Thread[] {
    const sorted = Sorter.sort([...this._threads.values()]);
    const mainThreadIndex = sorted.findIndex(i => i.name() === 'Main');
    if (mainThreadIndex >= 0) {
      const mainThread = sorted[mainThreadIndex];
      sorted.splice(mainThreadIndex, 1);
      sorted.splice(sorted.findIndex(i => i.name() === 'CrBrowserMain') + 1, 0, mainThread);
    }
    return sorted;
  }
}
