...
export class ConsoleSidebar extends UI.Widget.VBox {
  ...
  _appendGroup(
    ...
    const tagsMask = Common.Settings.Settings.instance().moduleSetting<string>('consoleShowLogsOfTag').get().split(',');
    const filter = new ConsoleFilter(name, parsedFilters, null, levelsMask, tagsMask as Protocol.Log.LogEntryTag[]);
