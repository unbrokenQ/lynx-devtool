...
export class TimelineController implements SDK.TargetManager.SDKModelObserver<SDK.CPUProfilerModel.CPUProfilerModel>,
  ...
  tracingFinished?(args: {success: boolean, result?: any}): void {
    this._client.loadingFinished?.(args);
  }
  ...
  traceNeedConversion(needConversion: boolean): void {
    this._traceNeedConversion = needConversion;
  }
  ...
  async _finalizeTrace(): Promise<void> {
    ...
    this._client.loadingComplete(this._tracingModel, this._traceNeedConversion);
  }
  ...
  eventsRetrievalProgress(progress: number | string, label?: string): void {
    this._client.loadingProgress(progress, label);
  }
}
